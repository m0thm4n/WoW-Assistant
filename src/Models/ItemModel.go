...
type ItemMongo struct {
    ...
    SubclassName string `binding:"required"`
    SubclassID int `binding:"required"`
    QualityType string `binding:"required"`
    QualityName string `binding:"required"`
    Level int `binding:"required"`
    RequiredLevel int `binding:"required"`
    ItemClassName string `binding:"required"`
    ItemClassID int `binding:"required"`
    InventoryTypeName string `binding:"required"`
    InventoryType string `binding:"required"`
    PurchasePrice int `binding:"required"`
    SellPrice int `binding:"required"`
    MaxCount int `binding:"required"`
    IsEquippable bool `binding:"required"`
    IsStackable bool `binding:"required"`
    AuctionID int `binding:"required"`
    Buyout int `binding:"required"`
    Quantity int `binding:"required"`
    UnitPrice int `binding:"required"`
    TimeLeft wowgd.TimeLeft `binding:"required"`
    Table    string `bson:"omitempty"`
}

func GetCollectionName(item *ItemMongo)  {
    colName = GetRealm(item)

    GetCollectionName(item)
}

func GetRealm(item *ItemMongo) string {
    switch item.Table {
    case "aegwynn":
        item.Table = "aegwynn"
    case "aerie peak":
...
func (item *ItemSQL) TableName() string {
  switch item.Table {
  case "aegwynn":
      item.Table = "aegwynn"
  case "aerie peak":
